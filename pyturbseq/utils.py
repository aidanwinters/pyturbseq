...
def generate_perturbation_matrix(
    ...
    if control_value not in feature_list:
        raise ValueError(
            f"Trying to pass 'control_value' of '{control_value}' to 'generate_perturbation_matrix' but not found in feature list"
        )
    ...
    if not keep_ref:
        ...
        perturbation_matrix = np.delete(
            perturbation_matrix, feature_dict[control_value], axis=1
        )
    ...
    return pd.DataFrame(
        perturbation_matrix, index=adata.obs.index, columns=feature_list
    )
...
def _get_target_change_single_perturbation(adata, gene, perturbed_bool, ref_bool):
    ...
    warnings.warn(
        "This function is deprecated. Please use _get_target_change_single_perturbation_indexed instead.",
        DeprecationWarning,
    )
    ...
    out["target_pct_change"] = (
        (target_gene_expression[perturbed_bool] - reference_target_mean)
        / reference_target_mean
    ) * 100
    out["target_zscore"] = (
        target_gene_expression[perturbed_bool] - reference_target_mean
    ) / reference_target_std
    out["target_log2fc"] = np.log2(
        (target_gene_expression[perturbed_bool] + 1) / (reference_target_mean + 1)
    )
...
def _get_target_change_single_perturbation_indexed(
    ...
    out["target_reference_std"] = reference_target_std
    out["target_pct_change"] = (
        (ptrb_gex - reference_target_mean) / reference_target_mean
    ) * 100
...
def calculate_target_change(
    ...
    **kwargs,
    ...
    if sum(duplicated_genes) > 0:
        raise ValueError(
            f"Duplicated gene names found in adata.var.index. Please remove duplicated gene names before running this function. \nDuplicated gene names found: {list(adata.var.index[duplicated_genes])}"
        )

    duplicated_obs = adata.obs.index.duplicated()
    if sum(duplicated_obs) > 0:
        raise ValueError(
            f"Observation names are not unique. To make them unique, call `.obs_names_make_unique` before running target change."
        )
    ...
    if not quiet:
        print(
            f"Computing target change for '{perturbation_column}' across {final_adata.shape[0]} cells..."
        )

    if check_norm:
        if not quiet:
            print("\tChecking if data is normalized to counts per cell...")
        if cells_not_normalized(final_adata):
            if not quiet:
                print("\t\tData is not normalized to counts per cell. Normalizing...")
            final_adata.layers["temp_counts"] = final_adata.X.copy()
            sc.pp.normalize_total(final_adata)
        else:
            if not quiet:
                print("\t\tData already normalized to counts per cell.")
    ...
    if not quiet:
        print(
            f"\tGenerating perturbation matrix from '{perturbation_column}' column..."
        )
    ...
    targets = [
        perturbation_gene_map[x] if perturbation_gene_map is not None else x
        for x in pm.columns
    ]
    ...
    metrics = [
        ...
        "target_reference_std",
    ]
    for m in metrics:
        final_adata.obsm[m] = (
            pd.DataFrame(index=final_adata.obs.index, columns=pm.columns)
            .fillna(0)
            .infer_objects(copy=False)
        )
    final_adata.obsm["target_gene"] = pd.DataFrame(
        index=final_adata.obs.index, columns=pm.columns
    )
    metrics += ["target_gene"]
    ...
    if not quiet:
        print("\tComputing target change for each cell...")

    for group, group_idx in tqdm(groups.items(), desc="Groups", disable=quiet):
        ref_bool = (
            pm.loc[group_idx].sum(axis=1) == 0
        ).values  # get the non perturbed cells for this grouping
        ...
        for perturbation in tqdm(
            ...
            target = (
                perturbation_gene_map[perturbation]
                if perturbation_gene_map is not None
                else perturbation
            )
            out = _get_target_change_single_perturbation_indexed(
                final_adata, target, group_idx, ref_idx
            )
            for m in metrics:
                if m == "target_gene":
    ...
    if not quiet:
        print("Target change calculation done. Storing in AnnData...")

    if not collapse_into_obs:
        if not quiet:
            print(
                f"\t'collapse_into_obs' is {collapse_into_obs}. Keeping metrics in .obsm..."
            )
    elif (
        sum(pm.sum(axis=1) > 1) == 0
    ):  # if only a single perturbation and specified to collapse into .obs
        if not quiet:
            print(
                f"\tNo cells with more than 1 perturbation. Adding metrics to .obs..."
            )
        ...
        if not quiet:
            print(f"\tMultiple perturbations found. Keeping metrics in .obsm...")
...
def cluster_adjacency(adata, method="leiden", inplace=True, **kwargs):
    ...
    if "adjacency" not in adata.obsm.keys():
        print(
            "No adjacency found at .obsm['adjacency']. Calculating adjacency matrix with default params..."
        )
    ...
    if method == "hdbscan":
        ...
        if inplace:
            ...
            adata.obs["adjacency_cluster"] = (
                adata.obs["adjacency_cluster"].astype("Int64").astype("str")
            )
            adata.obs["adjacency_cluster"] = adata.obs["adjacency_cluster"].replace(
                "-1", None
            )
        else:
            return labels
...
def calculate_edistances(
    ...
    def get_data(adata_subset):
        if use_X:
            return (
                adata_subset.X.toarray() if issparse(adata_subset.X) else adata_subset.X
            )
        else:
    ...
    for group in groups:
        ...
        results.append(
            {"perturbation": group, "edistance": edist, "n_pert": N, "n_control": M}
        )
...
def _zscore(
    ...
    if len(ref_inds) == 0:
        raise ValueError(
            f"ref_col '{ref_col}' and ref_val '{ref_val}' yielded no results"
        )
...
def zscore(
    adata: sc.AnnData, covariates: Optional[Union[str, List[str]]] = None, **kwargs
) -> np.ndarray:
...
def subsample_on_covariate(
    ...
    try:
        ...
        indices = (
            adata.obs.groupby(column, include_groups=False)
...
def subsample_on_multiple_covariates(
    ...
    assert len(adata.obs.index) == len(
        set(adata.obs.index)
    ...
    if min_cols:
        ...
        group_sizes = pd.merge(
            group_sizes[group_sizes.columns[:-1]], min_counts, on=other_cols, how="left"
        )
    else:
        # Determine the minimum number of samples for any label
    ...
    for _, row in group_sizes.iterrows():
        ...
        sampled_indices = np.random.choice(
            eligible_indices, int(row["count"]), replace=False
        )
...
def calculate_label_similarity(
    ...
    if subsample:
        ...
        if verbose:
            print(
                f"\tSubsampled data to have equal representation of labels. Total cells: {adata.n_obs}. Cells per label: {adata.n_obs / len(adata.obs[label_column].unique())}"
            )
    ...
    if subset:
        if subset > adata.n_obs:
            warnings.warn(
                f"Desired subset size ({subset}) is greater than the number of cells in the dataset ({adata.n_obs}). Continuing WITHOUT subsetting..."
            )
        else:
    ...
    within_label_pairs = [
        (unique_labels[I], unique_labels[I]) for I in range(len(unique_labels))
    ]
    ...
    if group_subset:
        ...
        if verbose:
            print(
                f"\tSubsetting # of across groups to approx. match # of within group comparisons"
            )
            print(
                f"\tTotal across group comparisons: {len(unique_labels) * (len(unique_labels) - 1) / 2}"
            )
    ...
    df["label1"] = np.concatenate(
        [[l1] * len(results[I]) for I, (l1, l2) in enumerate(all_pairs)]
    )
    df["label2"] = np.concatenate(
        [[l2] * len(results[I]) for I, (l1, l2) in enumerate(all_pairs)]
    )
