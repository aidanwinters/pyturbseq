...
def parse_CR_h5(
    ...
    if not os.path.exists(cellranger_h5_path):
        raise ValueError(
            f"Cellranger h5 file provided but file does not exist: {cellranger_h5_path}"
        )
    ...
    if pattern is not None:
        add_pattern_to_adata(
            adata, cellranger_h5_path, pattern, strict=True, quiet=quiet
        )
    ...
    if guide_call_csv is not None:
        ...
        if not os.path.exists(guide_call_csv):
            raise ValueError(
                f"Guide call CSV provided but file does not exist: {guide_call_csv}"
            )
        ...
        vp(
            f"Found sgRNA information for {len(inds)}/{adata.obs.shape[0]} ({round(len(inds) / adata.obs.shape[0] * 100, 2)}%) of cell barcodes"
        )
...
def add_CR_umi_metrics(adata: sc.AnnData) -> sc.AnnData:
    ...
    """
    df = pd.DataFrame(
        [parse_umi(x) for x in adata.obs["num_umis"]], index=adata.obs.index
    )
...
def parse_CR_flex_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if comma_fmt.any():
        df.loc[comma_fmt, "Metric Value"] = (
            df.loc[comma_fmt, "Metric Value"].str.replace(",", "").astype(float)
        )
